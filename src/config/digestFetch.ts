...
const parse = (raw: any, field: string, trim = true) => {
...
export class DigestFetch {
    ...
    constructor(user: string, password: string, options: any = {}) {
        this.user = user;
    ...
    async fetch(url: string, options: any = {}) {
        ...
        if (
            ...
            if (this.hasAuth) {
                ...
                if (
                    respFinal.status == 401 ||
                    respFinal.status == this.statusCode
                ) {
                    this.hasAuth = false;
                } else {
                    this.digest.nc++;
    ...
    addAuth(url: any, options: any) {
        ...
        const uri =
            _url.indexOf("/") == -1 ? "/" : _url.slice(_url.indexOf("/"));
        const method = options.method ? options.method.toUpperCase() : "GET";

        let ha1 = this.precomputedHash
            ? this.password
            : DigestFetch.computeHash(
                  this.user,
                  this.digest.realm,
                  this.password
              );
        ...
        if (this.digest.qop === "auth-int") {
            ...
            if (this.logger)
                this.logger.warn(
                    "Sorry, auth-int is not implemented in this plugin"
                );
...
algorithm="${this.digest.algorithm}",response="${response}",nc=${ncString},cnonce="${this.digest.cnonce}"`;
    ...
    async parseAuth(h: any) {
        ...
        this.digest.realm = (parse(h, "realm", false) || "").replace(
            /["]/g,
            ""
        );
    ...
    makeNonce() {
        ...
        for (let i = 0; i < this.cnonceSize; ++i) {
            uid +=
                this.nonceRaw[Math.floor(Math.random() * this.nonceRaw.length)];
        }
        return uid;
    }
